 ...
 * 1. It only implements uniquery, and the query constraint is hard-coded now
 * 2. It assumes all partitions need to be check
 * 3. It assumes it checks data at timestep 0
 * 4. It leaves out data candidate step
 * 5. It does not cached the partition metadata
 */
